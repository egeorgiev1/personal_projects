...
// Check if the image contains one and only one face
$imageBytes = file_get_contents($filePath);
$res = null;
try {
    $res = $rekognition->detectFaces([
        "Attributes" => [],
        "Image" => [
            "Bytes" => $imageBytes
        ]
...
if(count($res["FaceDetails"]) != 1) {
    handleLoginException(400, "Image must contain one face.");
}
...
$oldImageFilename = $result[0]["match_pic_name"];

// Get the old image faceid
$oldFaceId = $result[0]["face_id"];
...
// Upload image to the Amazon Rekognition collection
$res = null;
try {
    $res = $rekognition->indexFaces([
        'CollectionId' => 'main_collection',
        'DetectionAttributes' => [],
        'ExternalImageId' => $randomName,
        'Image' => [
            'Bytes' => $imageBytes,
        ],
    ]);
...
// Get the Amazon Rekognition faceid for the database
$faceId = $res["FaceRecords"][0]["Face"]["FaceId"];

// Change the profile picture in the database
$query = $db->prepare("
    UPDATE user_info SET match_pic_name=:profile_pic_name, face_id=:face_id WHERE user_id=:user_id
");

$user_id = $auth->id();
$query->bindParam(":user_id",$user_id);
$query->bindParam(":profile_pic_name",$randomName);
$query->bindParam(":face_id",$faceId);
...
// Delete the old image from the Amazon Rekognition collection if one was already set as a matchmaking picture
$res = null;
if($oldFaceId !== null) {
    try {
...
// TESTING: print all image names in the amazon rekognition collection
/*$res = null;
try {
